...
The station closest to the median temperature is the station with USAFID 7204458 in KY. The station closest to the median wind speed is the station with USAFID in WI. The station closest to the median atmostpheric pressure is the station with USAFID 722238 in AL. Hence, they do not coincide.
...
```{r}
mid_point <- met[, .(lon_50 = quantile(lon, probs= 0.5, na.rm=T), lat_50 = quantile(lat, probs = 0.5, na.rm = T)), by = STATE]
...
knitr::kable(tab, caption="Mean Temperature, Mean Wind Speed and Mean Atmospheric Pressure by Temperature Levels")
```
...
gam_mod <- gam(temp_50 ~ s(station_ave$wind.sp_50, bs="cr", k=20), data = station_ave)
summary(gam_mod)

```

From the residuals vs. fitted plot, we see that our residuals are not constant. Further, from the scale-location plot, we can check for constant variance. Because there's a positive trend in this plot, variance is not constant but instead, increasing. Further, we note from the residuals vs. leverage plot, there seems to be no potential influential points.


Taking a look at the adjusted $R^2$ value, we see that the linear model has a very low value at 0.01694; while the GAM model has a much higher adjusted $R^2$ value of 0.67. Hence, the GAM model is a better fit for the data. However, note that overfitting may be a possible concern for GAM. 

# Delivarables

-   Answer questions 1-5

-   https://github.com/annguyen1404/JSC370-labs.git
