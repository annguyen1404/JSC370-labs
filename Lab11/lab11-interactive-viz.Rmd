...
```{r}
plt1 <- ggplot(cv_states, aes(x=date, y=new_cases, colour=state))+
  geom_line()
...
plt2 <- ggplot(cv_states, aes(x=date, y=new_deaths, colour=state))+
  geom_line()
...
plt3 <- ggplot(cv_states, aes(x=date, y=new_deaths, colour=state))+
  geom_line() +
...
```{r}
cv_states_today %>%
  plot_ly(x=~pop_density, y=~cases, type='scatter',
          mode="markers", color=~state, size=~population,
...
cv_states_today %>%
  ...
  plot_ly(x=~pop_density, y=~cases, color=~state, type="scatter", mode="markers", size=~population, sizes = c(5, 70),marker=list(sizemode="diameter", opacity = 0.5))
...
cv_states_today %>%
  filter(state !="District of Columbia") %>%
  plot_ly(x=~pop_density, y=~cases, color=~state, type="scatter", mode="markers", size=~population, sizes = c(5, 70), 
...
plt4 <- cv_states_today %>%
  filter(state != "District of Columbia") %>%
  ggplot(aes(x=pop_density, y=deathsper100k, color=state,
             size=population)) +
  geom_point()
...
From the plot, it does not look like there is a strong correlation between population density and deaths.
...
Taking a look at Florida, we see that new cases peak around Aug 10 and Jan 4. The deaths are hard to see since they are so low relative to the number of cases but by hovering, I estimate the peaks to be at roughly Aug 12 and Jan 13. Hence, the delay is roughly 2-11 days.
...
California statnds out the most (there are a lot of light greens- i.e. high numbers  for that state).
